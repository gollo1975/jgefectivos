...
?>
 ...
 <?
    ...
    $consu="select empleado.nomemple,empleado.nomemple1,empleado.apemple,empleado.apemple1 from empleado where empleado.cedemple='$valor'";
    $resulta=mysql_query($consu)or die ("Consulat incorrecta");
    $reg=mysql_num_rows($resulta);
    if ($reg==0):
       ?>
       <script language="javascript">
          alert("El Documento no existe en la tabla? ")
          history.back()
       </script>
       <?
    else:
        ...
        ?>
          <center><h4>Listado de Entrega de Aportes</h3></center>
