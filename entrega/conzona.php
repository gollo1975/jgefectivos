         ...
         <td>
           <td><select name="campo" class="cajas">
                              <option value="0">Seleccione la Zona
                                <?
                                 $consulta_z="select zona.codzona,zona.zona from zona where estado='ACTIVA' order by zona.zona ";
...
elseif(empty($campo)):
   ...
   $variable="select zona.zona from zona where
                 zona.codzona='$campo'";
         $resultado=mysql_query($variable)or die("Error en la consulta de zonas  ");
        while($filas=mysql_fetch_array($resultado)):
             ?>
              <table border="0" align="center">
               <tr class="cajas">
                 <td><b>Zona:</b>&nbsp;<?echo $filas["zona"];?></td>
               </tr>
                 </table>
                <?
        endwhile;
        $con="select zona.zona,empleado.nomemple,empleado.nomemple1,empleado.apemple,empleado.apemple1,empleado.cedemple,contrato.fechainic,contrato.fechater from empleado,contrato,zona
        ...
        if ($reg==0):
          ?>
           <script language="javascript">
             alert("No hay aportes sociales en esta zona ?")
             history.back()
           </script>
          <?
        else:
          ...
          $fechaf=date("Y-m-d");
           echo ("<input type=\"hidden\" id=\"tActualizaciones\" name=\"tActualizaciones\" value=\"" . mysql_num_rows($resu) . "\">");
            while($filas=mysql_fetch_array($resu)):
               ...
               <tr class="cajas">
               <th><? echo $i;?></th>
               <td class="cajas"><a href="detalladoaporte.php?cedula=<? echo $filas["cedemple"];?>"><? echo $filas["cedemple"];?></a></td>
