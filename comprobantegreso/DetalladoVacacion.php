...
?>
 <script language="javascript">
   alert("Digite el Nro de la programación de pago de Vacaciones!")
   history.back()
</script>
<?else:
	...
	$resC=mysql_query($Con)or die("Error al buscar Empresa");
        ...
        /*fin consulta*/
         $ConV="select detalleprogravaca.*,programarvacacion.vlrpagado,zona.nitzona from detalleprogravaca,zona,programarvacacion
	where programarvacacion.idprogramavaca=detalleprogravaca.idprogramavaca and
	      detalleprogravaca.codzona=zona.codzona and 
          detalleprogravaca.idprogramavaca='$NroPago' order by detalleprogravaca.zona";
	$resN=mysql_query($ConV)or die("Error al buscar Vacaciones");
	$regN=mysql_num_rows($resN);
	if($regN != 0):
	  ?>
	  <center><h4><u>Comprobante de Egreso[Vacación]</u></h4></center>
	  <form action="GrabarPagoVacacion.php" name="f1" method="post" id="f1">
	     ...
	     while ($filas_s = mysql_fetch_array($resN)):
           ...
           ?>
	           <tr class="cajas">
                   <th><?echo $i;?></th>
	              <?
	              echo ("<td><input type=\"checkbox\" id=\"datos[" . $i . "]\" name=\"datos[" . $i . "]\" value=\"" . $filas_s['codvaca'] ."\" onClick=\"ActualizarSaldo()\"></td>");?>
	             <td><input type="text" value="<?echo $filas_s["codvaca"];?>" name="NroV[<? echo $i;?>]"id="NroV[<? echo $i;?>]" size="8" readonly class="cajas"></td>
