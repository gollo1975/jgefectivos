  ...
  <script language="javascript">
	...
	function QuitarFoco(obj)
	{
		...
		var TotalValor = 0;
                ...
                BaseAiu = document.getElementById("TipoBaseAiu").value;
		...
		for (j=1; j<=totalitem; j++){
			...
			document.getElementById("Total[" + j + "]").value= TotalValorPagar.toFixed(0);
                        if (j = 1){
                           TotalValorBase = document.getElementById("Total[" + j + "]").value;
                        }
                        if (j = 2){
                            document.getElementById("Cantidad[" + j +"]").value =1;
                            document.getElementById("VlrUnitario[" + j +"]").value = parseFloat(TotalValorPagar / BaseAiu);
                            document.getElementById("Total[" + j +"]").value = TotalValorPagar / BaseAiu;
                        }
                 }
            }
       function ActualizarImpuestos()
         ...
         for (k=1; k<=Total; k++){
             ...
             if(k = 1){
                 ValorSubotal = parseFloat(document.getElementById("Total[" + k +"]").value);
             }
             if(k=2){
             ...
             TotalPagar = parseFloat((ValorSubotal + ValorIva)-(ValorFuente + ValorReteIva));
             document.getElementById("SubtotalServicio").value = ValorSubotal.toFixed(0);
...
if(empty($CodZona)){
	...
	/*codigo de cobrozona*/
         $Sql="select item.concepto,item.basefte,item.codcom from item
	       where item.sgsst='SI' order by puc ASC";
	...
	<form action="GrabarFacturaNueva.php" method="post" id="Busqueda" name="Busqueda">
		...
		<input type="hidden" name="TipoBaseVenta" value="<?echo $TipoBaseVenta;?>" id="TipoBaseVenta">
                ...
                <input type="hidden" name="TipoBaseAiu" value="<?echo $TipoVlr;?>" id="TipoBaseAiu">
		<table border="0" align="center">
				...
				<tr>
					...
					<td><input type="text" name="NroServicio" value="<?echo $NroContrato;?>" size="20" maxlength="10"  class="cajas"onfocus="ColorFoco(this.id)" onblur="QuitarFoco(this.id)" id="NroServicio"></td>
	                                <?if($AplicaIva=='SI'){?>
	                                    <td><b>Iva:</b></td>
					...
					<td><input type="text" name="Fecha_Vcto" value="<?echo date('Y-m-d')?>" size="26" maxlength="10" class="cajas"onfocus="ColorFoco(this.id)" onblur="QuitarFoco(this.id)" id="Fecha_Vcto"></td>
                                ...
                                <tr>
		                            ...
		                            <td colspan="15"><textarea name="observacion" cols="64" rows="3" class="cajas" onfocus="ColorFoco(this.id)" onblur="QuitarFoco(this.id)" id="observacion" style="width: 436px">IMPLEMENTACION DEL SGSST 2016</textarea></td>
		                       </tr>
                               <table border="0" align="center" >
				<tr class="cajas">
				...
				$i=1;
					...
					<th><?echo $i;?></th>
					<?
                                        echo ("<td><input type=\"checkbox\" id=\"Vector[" . $i . "]\" name=\"Vector[" . $i . "]\" value=\"" . $filas["codcom"] ."\"  onclick=\"ActualizarImpuestos()\"></td>");?>
