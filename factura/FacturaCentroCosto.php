...
if(empty($Vector)){
  ?>
  <script language="javascript">
   alert("Favor chequear el centro de costo que va a facturar.!")
   history.back()
  </script>
<?
}else{
	...
	/*codigo que busca la zona*/
      	$SqlZona="select zona.zona,zona.iva,zona.vlrfte,zona.vlriva,zona.porcre,sucursal.dian,sucursal.rango,sucursal.rango2 from sucursal,zona,zonacosto
		where  sucursal.codsucursal=zona.codsucursal and
                       zona.codzona=zonacosto.codzona and
	...
	/*codigo de cobrozona*/
        ...
        for ($i=1; $i <= $Total; $i++){
             if ($Vector[$i] != "" ){
	       $Sql="select zonacosto.codigo,zonacosto.contador,zonacosto.desde,zonacosto.hasta,detalladozona.periocidad,zonacosto.subtotalsinadmon,zonacosto.admon,zonacosto.iva,zonacosto.subtotal,zonacosto.grantotal,zonacosto.contador from zonacosto,detalladozona
		        where  zonacosto.codzona=detalladozona.codzona and
		               zonacosto.codcosto='$Vector[$i]' and
	...
	if($Periodo==7):
	   ...
	   if($Periodo==10):
        ...
        if($SubtotalServicio > 0){;
	        /*fin codigo*/
		if($Sw !=0){
	              if($AjusteFacturaCentro=='NO'){
	                 $ValorBase = round($SubtotalServicio / $TipoVlr);
	                 $Item="select item.* FROM item
		                 where item.misional='SI' order by item.puc ASC";
	              }else{
	                  $ValorBase = round(($SubtotalServicio + $ValorAjusteCentro)/ $TipoVlr);
	                  $Item="select item.* FROM item
		                 where item.ajustefacturacentro='SI' order by item.puc ASC";
	              }
			...
			<form action="GrabarFacturaNueva.php" method="post" id="Busqueda" name="Busqueda">
				<input type="hidden" name="AplicaIva" value="<?echo $AplicaIva;?>" id="AplicaIva">
	                        ...
	                        <input type="hidden" name="ValorAjusteCentro" value="<?echo $ValorAjusteCentro;?>" id="ValorAjusteCentro">
	                        <input type="hidden" name="AjusteFacturaCentro" value="<?echo $AjusteFacturaCentro;?>" id="AjusteFacturaCentro">
					...
					<th><b>Item</b></td></th><th><b></b></th><th><b>Código</b></th><th><b>Concepto</b></th><th><b>Cant.</b></th><th><b>Vlr_Unit.</b></th><th><b>Dcto</b></th><th><b>Total</b></th>
	                                ...
	                                <input type="hidden" name="Total" value="<?echo $TipoBaseVenta;?>" id="TipoBaseVenta">
					<?
					$i=1;
						...
						<?
						        ...
						        echo ("<td><input type=\"checkbox\" id=\"Vector[" . $i . "]\" name=\"Vector[" . $i . "]\" value=\"" . $filas['codcom'] ."\"></td>");?>
							        ...
							        echo ("<td><input type=\"checkbox\" id=\"Vector[" . $i . "]\" name=\"Vector[" . $i . "]\" value=\"" . $filas['codcom'] ."\"></td>");?>
			                                        ...
			                                        <td><input type="text" value="<?echo $ValorAjusteCentro;?>" name="VlrUnitario[<? echo $i;?>]"id="VlrUnitario[<? echo $i;?>]"size="13" class="cajas" style="text-align:right;background-color:#70EBCF"></td>
			                                        <td><input type="text" value="0" name="Dcto[<? echo $i;?>]"id="Dcto[<? echo $i;?>]"size="4" class="cajas"></td>
			                                        <td><input type="text" value="<?echo $ValorAjusteCentro;?>" name="Total[<? echo $i;?>]"id="Total[<? echo $i;?>]"size="13" class="cajas" style="text-align:right;background-color:#70EBCF"></td>
		                                           <?
	                                                   }
	                                       }
					...
					endwhile;
	                                ...
	                                $ValorSubtotal = $SubtotalServicio + $ValorAjusteCentro;
	                                if($TipoFactura==1){
	         ...
	         <?
		      ...
		      ?>
			  <script language="javascript">
			   alert("Este centro de costos no tiene personal para factura.!")
			   history.back()
			  </script>
	              <?
	         }
                ...
                ?>
			  <script language="javascript">
			   alert("No hay registros para facturar en este centro de Costos.!")
			   history.back()
			  </script>
	              <?
          }
