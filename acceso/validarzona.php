...
<html>
<head>
  <title>Coopiser-Cambio de Clave</title>
...
if (!isset($actual)):
  ...
  <center><h4><u>Cambio de Clave</u></h4></center>
    <form action="" method="post" id="matusuario">
     <td><input type="hidden" name="fechaven" value="<?echo $fechaven;?>" </td>
  ...
  if($clave == $actual):
     if($nueva == $confirmar):
         if($clave != $nueva):
                 ...
                 $nueva=strtoupper($nueva);
                  $con="update accesozona set clave='$nueva',fechaven='$fechaven' + interval 45 day where usuario='$usuario' and cedula='$Documento'";
                  echo $con;
                  $resultado=mysql_query($con)or die("Error al actualizar datos de la clave");
                  $re=mysql_affected_rows();
                  $consulta="select accesozona.codzona from accesozona where usuario='$usuario' and clave='$nueva' and cedula='$Documento'";
                  $resultado=mysql_query($consulta)or die ("Error al validar el usuario de la tabla!");
