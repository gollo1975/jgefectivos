...
if (empty($valor)):
 ...
 $con="select  decentro.codsala from salario,decentro
        where salario.codsala=decentro.codsala and
              salario.codsala='$codigo'";
 $re=mysql_query($con)or die("Error de Busqueda de item de salario");
 $reg=mysql_num_rows($re);
 if ($reg!=0):
    $consulta="update decentro set salario='$valor' where decentro.codsala='$codigo' ";
